...
char w;
 ...
 static char * test_bar() {
  ...
  static char * test_NewNameTest() {
     mu_assert("Error, the change of name has failed", ItemName == NewNameTest);
    return 0;
 }

  static char * test_NewCodeTest() {
     mu_assert("Error, the change of code has failed", ItemCode == NewCodeTest);
    return 0;
 }

  static char * test_NewRateTest() {
     mu_assert("Error, the change of rate (= price) has failed", ItemRate == NewRateTest);
    return 0;
 }

 static char * test_NewQuantTest() {
     mu_assert("Error, the change of quantity has failed", ItemQuant == NewQuantTest);
    return 0;
 }
...
void d_mainmenu()
{
 ...
 for (i=0;i<=6;i++){
  ...
  if (count>0) // This condition is just to make sure we don't run the test in the beginning
    ...
    if(toupper(getche())=='Y'){
     ...
     switch(choice){
      ...
      case 2:
       ...
       strcpy(item.code,y);
       c_code(w);
       strcpy(NewCodeTest,w);
       size=sizeof(item);
       fseek(file,-size,SEEK_CUR);fwrite(&item,sizeof(item),1,file);
       //// TESTING THE CODE CHANGE
              ItemCode = item.code;
     result = test_NewCodeTest();
     if (result != 0) {
         gotoxy(27,34);printf("%s\n", result);
     }
     if (item.code == NewCodeTest)
        {
           gotoxy(27,32);printf("--- It's OK, test about editing code passed---");
        }
     ...
     if (item.rate == NewRateTest)
        {
           gotoxy(27,32);printf("--- It's OK, test about editing rate passed---");
        }
        tests_run = tests_run + 1;
