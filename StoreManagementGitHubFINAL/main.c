...
int TestError = 0; // variable that becomes 1 when a testing detects an error
...
char w;
  ...
  static char * test_NewRateTest() { // Test for the add function
     mu_assert("Error, the choice of a rate (= price) has failed", ItemRate == NewRateTest);
    return 0;
 }

 static char * test_NewQuantTest() {  // Test for the edit function
     mu_assert("Error, the editing of quantity has failed", ItemQuant == NewQuantTest);
    return 0;
 }

  static char * all_tests() {
     mu_run_test(test_NewRateTest);
     mu_run_test(test_NewQuantTest);
     return 0;
 }
...
void d_mainmenu()
{
 ...
 for (i=0;i<=6;i++){
  ...
  if (count>0) // This condition is just to make sure we don't display the test results in the beginning
               // but only when the functions to be tested have been used
  {
           if (TestError==1)
           {
               printf("An error was found while testing");
               printf("  "); // just to improve readability
           }
...
void highlight(int no,int count) // function that just allows to see where we are by replacing " Quantity " with " - Quantity "
{
 if (no==4){
  //textbackground(11);
  //textcolor(0);
...
void add ()
{
 ...
 while(toupper(getche())=='Y'){
  ...
  c_code(y);
  strcpy(item.code,y);
  gotoxy(22,28);printf("Enter rate of the item (twice with 'enter' after each):");
  scanf("%f",&item.rate);

  scanf("%f",&NewRateTest);

      ////// TESTING THE CHOICE A OF RATE FOR NEW ARTICLE
         ItemRate = item.rate;
      result = test_NewRateTest();
     if (result != 0) { // TEST FAILED
         gotoxy(22,37);printf("%s\n", result);
         TestError = 1;
     }
     if (item.rate == NewRateTest) // TEST SUCCEEDEED
        {
           gotoxy(22,37);printf("--- OK, function ADD (rate) works correctly ---");
        }
    ...
    if(toupper(getche())=='Y'){
     ...
     switch(choice){
      ...
      case 3:
       ...
       gotoxy(35,18);printf("EDIT RECORDS");
       gotoxy(25,24);
       printf(" enter new rate (twice, with enter after each): ");
       scanf("%f",&item.rate);
       size=sizeof(item);
       fseek(file,-size,SEEK_CUR);fwrite(&item,sizeof(item),1,file);
      ...
      case 4:
       ...
       gotoxy(35,18);printf("EDIT RECORDS");
       gotoxy(25,24);
       printf(" enter new quantity (twice with 'enter' after each): ");
       scanf("%d",&item.quantity);
       scanf("%d",&NewQuantTest);
       size=sizeof(item);
       fseek(file,-size,1);fwrite(&item,sizeof(item),1,file);
        ///////// TESTING THE QUANTITY CHANGE
       ItemQuant = item.quantity;
      result = test_NewQuantTest();
     if (result != 0) { //// TEST FAILED
         gotoxy(27,34);printf("%s\n", result);
         TestError = 1;
     }
     if (item.quantity == NewQuantTest) //// TEST SUCCEEDED
        {
           gotoxy(27,32);printf("--- It's OK, test about editing quantity passed---");
        }
        tests_run = tests_run + 1;
     }
        ////////
...
{
 FILE *file;
 char xTest [2]; // variable declared to test this function
xTest[0] = x[0];
xTest[1] = x[1];
 int flag=1;
 file=fopen("record.txt","rb");
 ...
 while (fread(&item,sizeof (item),1,file)){
     ...
     ////////// TESTING THE CHECK FUNCTION (DOES THE CODE THE USER ENTERS REALLY EXISTS ?)
  if(strcmp(item.code,x)==0){
      if (xTest[0]==  item.code[0] && xTest[1]==  item.code[1]) // We assume codes from 0 to 99
                                                                // so checking the two first digits is OK
      {
          gotoxy(22,38);printf("OK, the 'check' function works correctly");
          tests_run = tests_run + 1;
      }
      else {
          gotoxy(22,38);
          printf("Error, this function hasn't detected that the code doesnt exist");
          tests_run = tests_run + 1;
          TestError = 1;
          }
   /////////////
...
/*function to display box*/ // JUST THERE TO DISPLAY DEPARTMENTAL STORE ETC...
